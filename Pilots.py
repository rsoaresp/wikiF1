...
class Formula1():
    ...
    def popTable(self):
        ...
        temp = self.table['Championships'].str.split(' ');
        
        self.table['Championships'] = temp.str.get(0);
        self.table['Championship years'] = temp.str[1:].apply(np.ravel).map(lambda x: ''.join(x));
        self.table['Championship years'] = self.table['Championship years'].str.replace(',', ', ');
        ...
        colsConvert = ['Entries', 'Starts', 'Poles', 'Wins', 'Podiums', 'Fastest laps',
                       'Points[note]', 'First season', 'Last season', 'Championships'];
                       
        self.table[colsConvert] = self.table[colsConvert].apply(pd.to_numeric,
                                                                errors = 'coerce',
                                                                axis = 1);   
